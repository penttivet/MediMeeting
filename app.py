...
app = Flask(__name__)
app.secret_key = os.environ.get("SECRET_KEY", "medimeeting-secret-key-12345")
...
<style>
  ...
  * { margin:0; padding:0; box-sizing:border-box; -webkit-tap-highlight-color:transparent; }
  ...
  input, select { background:var(--surface2); border:1px solid var(--border); border-radius:10px; padding:13px 16px; color:var(--text); font-family:'DM Sans',sans-serif; font-size:15px; width:100%; outline:none; transition:border-color 0.2s; margin-bottom:10px; }
  ...
  .result-section.visible { display:flex; flex-direction:column; gap:12px; margin-top:16px; }
  .summary-box { background:var(--surface2); border-radius:10px; padding:14px; font-size:14px; line-height:1.6; color:var(--text); border:1px solid var(--border); white-space:pre-wrap; max-height:200px; overflow-y:auto; }
</style>
</head>
<body>
<div class="header">
  ...
  <div id="meeting" class="tab-content active">
    ...
    <div class="card">
      ...
      <div class="record-section">
        ...
        <div class="record-status" id="meetingStatus">Press to record</div>
      </div>
    </div>
    <button class="btn btn-primary" id="meetingGenBtn" onclick="generate('meeting')" disabled>✨ Create summary</button>
    <div class="result-section" id="meetingResult">
      <div class="card">
  ...
  <div id="call" class="tab-content">
    ...
    <div class="card">
      ...
      <div class="record-section">
        ...
        <div class="record-status" id="callStatus">Press to record</div>
      </div>
    </div>
    <button class="btn btn-primary" id="callGenBtn" onclick="generate('call')" disabled>✨ Create summary</button>
    <div class="result-section" id="callResult">
      <div class="card">
        <div class="card-title">Summary</div>
...
async function generate(type) {
  ...
  formData.append('title', type === 'meeting' ? document.getElementById('meetingTitle').value : document.getElementById('callTopic').value);
...
function reset(type) {
  ...
  document.getElementById(type + 'Timer').classList.remove('visible');
  document.getElementById(type + 'Status').textContent = 'Press to record';
}
</script>
</body>
</html>"""
...
<style>
  @import url('https://fonts.googleapis.com/css2?family=DM+Serif+Display&family=DM+Sans:wght@300;400;500;600&display=swap');
  :root {{ --bg:#0f1117; --surface:#1a1d27; --surface2:#22263a; --accent:#4f8ef7; --accent2:#7c6af7; --text:#e8eaf0; --text2:#8b90a8; --border:rgba(255,255,255,0.07); }}
  * {{ margin:0; padding:0; box-sizing:border-box; }}
  ...
  .link {{ text-align:center; font-size:13px; color:var(--text2); }}
...
def register():
    ...
    if request.method == "POST":
        ...
        if redis_get(f"mm:user:{email}"):
            ...
            redis_set(f"mm:user:{email}", {"name": name, "email": email, "password": hash_password(password), "status": "pending", "created": datetime.now().isoformat()})
            msg = '<div class="msg error">✅ Registered! Waiting for approval</div>'
    form = '''<form method="POST">
    ...
    try:
        resp = requests.post(
            ...
            json={
                ...
                "max_tokens": 500,
                "system": "Summarize this meeting/call in 2-3 sentences focusing on key points and decisions.",
...
def ensure_admin():
    try:
        ...
        if not existing:
            redis_set(f"mm:user:{admin_email}", {"name": "Admin", "email": admin_email, "password": admin_hash, "status": "approved", "created": datetime.now().isoformat()})
